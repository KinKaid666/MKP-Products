...
use constant OHIS_INSERT_STATEMENT => qq( insert into onhand_inventory_reports ( sku, report_date, source_name, condition_name, quantity )
                                                                        values (   ?,           ?,           ?,              ?,        ? ) ) ;
...
&GetOptions(
    ...
    "report-date=s"  => \$options{report_date},
...
die "You must provide a --filename and a --report-date." if (not defined $options{filename} or not defined $options{report_date}) ;
die "Date format must be YYYY-MM-DD." if (not $options{report_date} =~ m/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/) ;
...
# Insert each order
{
    ...
    my $i_stmt = $dbh->prepare(${\OHIS_INSERT_STATEMENT}) ;
    foreach my $ohi (@ohis)
    {
...
--filename     the filename that contains the inventory report
