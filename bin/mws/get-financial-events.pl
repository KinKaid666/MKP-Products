...
# | giftwarp_charges     | decimal(13,2)    | NO   |     | NULL              |                             |
# | giftwarp_charges_tax | decimal(13,2)    | NO   |     | NULL              |                             |
...
my %options ;
$options{username} = 'mkp_loader2'     ;
$options{password} = 'mkp_loader_2018' ;
$options{database} = 'mkp_products2'   ;
...
if( $options{debug} or $options{timing} )
{
    print "Debug  = $options{debug}\n" ;
    print "Timing = $options{timing}\n" ;
}

# pure debug; remove before prod
if( 0 )
{
...
{
    print STDERR "You must define a --start and --end or one or the other and --duration\n" ;
    &usage_and_die(1) ;
}
...
{
    $start = UnixDate(DateCalc(ParseDate($options{end})  , "- 1 $options{duration}"),"%Y-%m-%d") ;
}
...
if( Date_Cmp(DateTime->now()->add(minutes => -2), $end) < 0 )
{
    print STDERR "ERROR: Date window must be at least 2 minutes old\n" ;
    exit(1) ;
}

print "Window requested is $start to $end\n" if $options{debug} ;
...
my $groupReq = $mws->ListFinancialEventGroups(FinancialEventGroupStartedAfter  => $start,
...
while(1)
{
    ...
    foreach my $fGroup (@{$groupReq->{FinancialEventGroupList}->{FinancialEventGroup}})
    {
        ...
        while(1)
        {
            ...
            if( exists $fegs->{$fGroup->{FinancialEventGroupId}}->{FinancialEvents} )
            {
                foreach my $list (keys %{$req->{FinancialEvents}})
                {
                    ...
                    {
                        if( exists $fegs->{$fGroup->{FinancialEventGroupId}}->{FinancialEvents}->{$list} and
                            exists $fegs->{$fGroup->{FinancialEventGroupId}}->{FinancialEvents}->{$list}->{$name} )
                        {
                            #
                            # Return value isn't an array if there is only one value; so ensure it's always an array
                            &force_array($fegs->{$fGroup->{FinancialEventGroupId}}->{FinancialEvents}->{$list}->{$name}) ;
                            if( ref $req->{FinancialEvents}->{$list}->{$name} eq "ARRAY" )
                            {
                                foreach my $element (@{$req->{FinancialEvents}->{$list}->{$name}})
                                {
                            ...
                            {
                                push @{$fegs->{$fGroup->{FinancialEventGroupId}}->{FinancialEvents}->{$list}->{$name}}, $req->{FinancialEvents}->{$list}->{$name} ;
                            }
                        }
                        else
                        {
...
open my $out, '>', 'a.out' or die $!;
print $out Dumper($fegs) . "\n" ;
}

open my $in, '<', 'a.out' or die $!;
my $fegs ;
{
    local $/;    # slurp mode
    $fegs = eval <$in>;
                    ...
                    foreach my $item (@{&force_array($shipment->{ShipmentItemList}->{ShipmentItem})})
                    {
                        ...
                        if( exists $item->{ItemChargeList} and exists $item->{ItemChargeList}->{ChargeComponent} )
                        {
                            ...
                            foreach my $fee (@{&force_array($item->{ItemChargeList}->{ChargeComponent})})
                            {
                                ...
                                else                                                { die "unknown fee . $fee->{ChargeType}" ; }
                            }
                        }
...
# DB Load function
{
    ...
    foreach my $feg_id (keys %$financialEventGroups)
    {
        ...
        if( $s_sth->rows > 0 )
        {
            print STDERR "Skipping existing feg $feg_id\n" ;
            next ;
        }
