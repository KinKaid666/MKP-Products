--
-- Create the database
CREATE DATABASE IF NOT EXISTS mkp_products2 ;
...
DELIMITER //
CREATE TRIGGER order_sources_create_trigger BEFORE INSERT on order_sources
FOR EACH ROW 
BEGIN
    set NEW.creation_user = USER() ;
    set NEW.latest_user = USER() ;
END //
CREATE TRIGGER order_sources_update_trigger BEFORE UPDATE on order_sources
FOR EACH ROW 
BEGIN
    set NEW.latest_user = USER() ;
END //
...
(
   ...
   ,giftwarp_charges             DECIMAL(13,2)    NOT NULL                                                      -- "product sales"
   ,giftwarp_charges_tax         DECIMAL(13,2)    NOT NULL                                                      -- "product sales"
